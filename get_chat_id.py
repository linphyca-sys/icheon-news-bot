...
"""
텔레그램 chat_id 확인 도우미.
- 개인 채팅: 봇에게 아무 메시지나 먼저 보낸 뒤 실행
- 채널: 봇을 채널 관리자로 추가하고 채널에 아무 글이나 올린 뒤 실행
"""
...
if not chats:
    print("수신된 메시지가 없습니다. 봇에게 메시지를 보내거나 채널에 글을 올린 뒤 다시 실행하세요.")
else:
    print("발견된 chat_id:")
    for cid, name in chats.items():
        print(f"  {cid}  ({name})")
    print("\n채널 id는 보통 -100으로 시작하는 음수입니다. 이 값을 .env의 TELEGRAM_CHAT_ID에 넣으세요.")
